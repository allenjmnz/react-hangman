...
export class Game extends Component {
  ...
  componentDidUpdate = () => {
    ...
    if (complete) {
      ...
      } else if (this.state.questionNumber === (this.state.questionsAndAnswers.length - 2) && this.state.gameCompleted === false) { 
        this.setState({
          gameCompleted: true,
          currOp: 0,
  ...
  render() {
    ...
    return (
      <div className="game">
        {this.state.triggerConfetti ? <Confetti height={document.querySelector('.game').offsetHeight} width={document.querySelector('.game').offsetWidth} /> : null}
        <h2>Juego</h2>
